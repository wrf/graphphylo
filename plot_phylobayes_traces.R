...
rrrange  = range( pretty( range( c( trace1dat[,6],trace2dat[,6]) ) ) )
} else {
...
pdf(file=outputfile, width=11, height=11)
par(mfrow=c(2,2), mar=c(4.5,4.5,4,2) )
...
text(maxlength*0.75, llrange[1] + diff(llrange) * 0.6, paste("Final iterations =",tracemax) , cex=1.5)
legend(maxlength*0.65, llrange[1] + diff(llrange) * 0.4, legend=c("Trace 1", "Trace 2"), col=c("#f1a340", "#0571b0"), lwd=2, cex=1.4)
...
legend(maxlength*0.65, serange[1] + diff(serange) * 0.9, legend=c("Trace 1", "Trace 2"), col=c("#f1a340", "#0571b0"), lwd=2, cex=1.4)
...
legend(maxlength*0.65, lenrange[1] + diff(lenrange) * 0.4, legend=c("Trace 1", "Trace 2"), col=c("#f1a340", "#0571b0"), lwd=2, cex=1.4)

### MAKE DIFFERENT PLOTS FOR RRENT AND ALPHA
if (dim(trace1dat)[2] < 10) {
plot( trace1dat[,1], trace1dat[,6], type='l', xlim=c(0, maxlength), ylim=rrrange, lwd=3, col="#f1a34099", xlab="Iterations", ylab="alpha" , main="", cex.lab=1.4 )
...
legend(maxlength*0.65, rrrange[1] + diff(rrrange) * 0.9, legend=c("Trace 1", "Trace 2"), col=c("#f1a340", "#0571b0"), lwd=2, cex=1.4)
} else {
